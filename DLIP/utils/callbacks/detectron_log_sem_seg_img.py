...
class DetectronLogSemSegImgCallback(Callback):
    """
    ...
    def on_test_start(
        ...
        for ix in range(min(self.num_img_log,len(trainer.datamodule.test_dataset))):
            ...
            pred_mask = (torch.sigmoid(prediction[0]["sem_seg"])>0.5).cpu().squeeze().numpy()
            gt_mask = batch[0]["sem_seg"].cpu().squeeze().numpy()
